...
{
$sales = Penjualan::orderBy('created_at', 'desc')->paginate(10);
    ...
    $quantity = $request->has('quick_add') ? 1 : $request->quantity;
    ...
    $selisih = $request->quantity - $itempenjualan->kuantitas;
        ...
        if ($selisih > 0) {
            if ($produk->stok < $selisih) {
                return redirect()
                    ->route('penjualan.create')->with('errors', 'Stok tidak mencukupi');
            }
